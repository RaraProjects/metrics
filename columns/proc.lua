...
Column.Proc.Crit_Rate = function(player_name, damage_type, justify)
    ...
    if damage_type == DB.Enum.Values.COMBINED then
        local melee_crits     = DB.Data.Get(player_name, Column.Trackable.MELEE, DB.Metric.CRITICAL_COUNT)
        local melee_hits  = DB.Data.Get(player_name, Column.Trackable.MELEE, DB.Metric.HIT_COUNT)
        local ranged_crits    = DB.Data.Get(player_name, Column.Trackable.RANGED, DB.Metric.CRITICAL_COUNT)
...
Column.Proc.Crit_Damage = function(player_name, damage_type, percent, justify)
    local crit_damage
    if damage_type == DB.Enum.Values.COMBINED then
        local melee_crits  = DB.Data.Get(player_name, Column.Trackable.MELEE,  DB.Metric.CRITICAL_DAMAGE)
        local ranged_crits = DB.Data.Get(player_name, Column.Trackable.RANGED, DB.Metric.CRITICAL_DAMAGE)
...
Column.Proc.Crit_Average = function(player_name, damage_type, justify)
    ...
    if damage_type == DB.Enum.Values.COMBINED then
        local melee_crits       = DB.Data.Get(player_name, Column.Trackable.MELEE,  DB.Metric.CRITICAL_DAMAGE)
        local melee_crit_count  = DB.Data.Get(player_name, Column.Trackable.MELEE,  DB.Metric.CRITICAL_COUNT)
        local ranged_crits      = DB.Data.Get(player_name, Column.Trackable.RANGED, DB.Metric.CRITICAL_DAMAGE)
        local ranged_crit_count = DB.Data.Get(player_name, Column.Trackable.RANGED, DB.Metric.CRITICAL_COUNT)
        crit_damage = melee_crits + ranged_crits
...
Column.Proc.Kick_Rate = function(player_name)
    local kick_count = DB.Data.Get(player_name, Column.Trackable.MELEE_KICK, DB.Metric.ATTEMPTS)
    local melee_cycles = DB.Data.Get(player_name, Column.Trackable.MELEE, DB.Metric.MELEE_ROUNDS)
...
Column.Proc.Deaths = function(player_name, justify)
    local death_count = DB.Data.Get(player_name, Column.Trackable.DEATH, DB.Metric.ATTEMPTS)
...
Column.Proc.Distance_Correction = function(player_name, correction_type, justify)
    local ranged_shots = DB.Data.Get(player_name, Column.Trackable.RANGED, DB.Metric.ATTEMPTS)
