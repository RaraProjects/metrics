...
m.Data = {
	Parse = {}, 				-- [player:mob] Holds all of the damage data that the parser uses index is 
	Trackable_Data = {},		-- [trackable][player_name] Keeps track of which skills have been initialized
...
------------------------------------------------------------------------------------------------------
-- Initializes an "actor:target" combination in the primary data node.
...
---@param player_name? string used for maintaining various player indexed tables.
---@return boolean
------------------------------------------------------------------------------------------------------
m.Init.Init_Data = function(index, player_name)
	if not index then return false end
	if m.Data.Parse[index] then return false end
...
---@return boolean
------------------------------------------------------------------------------------------------------
m.Init.Init_Data_Catalog = function(index, player_name, trackable, action_name)
	if not index or not player_name or not trackable or not action_name then return false end

	m.Init.Init_Data(index, player_name)

	-- Don't want to overwrite action_name node if it is already built out
	if m.Data.Parse[index][trackable][m.Enum.Node.CATALOG][action_name] then return false end
	...
	-- Initialize tracking tables
	if not m.Data.Trackable_Data[trackable] then m.Data.Trackable_Data[trackable] = {} end
	if not m.Data.Trackable_Data[trackable][player_name] then m.Data.Trackable_Data[trackable][player_name] = {} end
...
---@param trackable string a tracked item from the trackable list.
---@param value number value to be logged.
---@param action_name string the name of the action to be cataloged.
------------------------------------------------------------------------------------------------------
m.Update.Catalog_Damage = function(player_name, mob_name, trackable, value, action_name)
    local index = m.Util.Build_Index(player_name, mob_name)
    m.Init.Init_Data_Catalog(index, player_name, trackable, action_name)
    ...
    if trackable ~= m.Enum.Trackable.HEALING then
    	m.Update.Data(m.Enum.Mode.INC, value, audits, m.Enum.Trackable.TOTAL, m.Enum.Metric.TOTAL)
		if trackable ~= m.Enum.Trackable.SC then
			m.Update.Data(m.Enum.Mode.INC, value, audits, m.Enum.Trackable.TOTAL_NO_SC, m.Enum.Metric.TOTAL)
		end
    end

    -- Overall data, min and max
    m.Update.Data(m.Enum.Mode.INC, value, audits, trackable, m.Enum.Metric.TOTAL)

    if value > 0 and value < m.Get.Data(player_name, trackable, m.Enum.Metric.MIN) then
		m.Update.Data(m.Enum.Mode.SET, value, audits, trackable, m.Enum.Metric.MIN)
	end

    if value > m.Get.Data(player_name, trackable, m.Enum.Metric.MAX) then
		m.Update.Data(m.Enum.Mode.SET, value, audits, trackable, m.Enum.Metric.MAX)
	end

    -- Catalog data, min and max
	-- 'count' gets incremented in the packet handling.
    m.Update.Catalog_Metric(m.Enum.Mode.INC, value, audits, trackable, action_name, m.Enum.Metric.TOTAL)

    if value > 0 and value < m.Get.Catalog(player_name, trackable, action_name, m.Enum.Metric.MIN) then
    	m.Update.Catalog_Metric(m.Enum.Mode.SET, value, audits, trackable, action_name, m.Enum.Metric.MIN)
    end

    if value > m.Get.Catalog(player_name, trackable, action_name, m.Enum.Metric.MAX) then
    	m.Update.Catalog_Metric(m.Enum.Mode.SET, value, audits, trackable, action_name, m.Enum.Metric.MAX)
    end
end
...
m.Update.Catalog_Metric = function(mode, value, audits, trackable, action_name, metric)
	...
	m.Data.Trackable_Data[trackable][player_name][action_name] = true
...
m.Util.Populate_Catalog_Damage_Table = function(player_name)
	m.Data.Catalog_Damage_Race = {}
	for action_name, _ in pairs(m.Data.Trackable_Data[Focus.Settings.Trackable][player_name]) do
