...
H.Melee_Def.Parse = function(result, actor_name, target_name, owner_mob)
    ...
    if not owner_mob then
        ...
        if not action_taken then
            DB.Data.Update(DB.Update_Mode.INC, 1, audits, DB.Trackable.DEF_MELEE, DB.Metric.HIT_COUNT)
            DB.Data.Update(DB.Update_Mode.INC, 1, audits, DB.Trackable.DEF_UNMITIGATED, DB.Metric.HIT_COUNT)
...
H.Melee_Def.Totals = function(audits, damage, no_damage)
    ...
    DB.Data.Update(DB.Update_Mode.INC, 1,      audits, trackable, DB.Metric.ATTEMPTS) -- Melee attempts against entity.
...
H.Melee_Def.Parry = function(audits, message_id)
    local parry = false
    DB.Data.Update(DB.Update_Mode.INC, 1, audits, DB.Trackable.DEF_PARRY, DB.Metric.ATTEMPTS)
    if message_id == Ashita.Enum.Message.PARRY then
        DB.Data.Update(DB.Update_Mode.INC, 1, audits, DB.Trackable.DEF_PARRY, DB.Metric.HIT_COUNT)
        parry = true
    end
    return parry
end
...
H.Melee_Def.Third_Eye = function(audits, message_id)
    ...
    DB.Data.Update(DB.Update_Mode.INC, 1, audits, DB.Trackable.DEF_THIRD_EYE_ANTICIPATION, DB.Metric.ATTEMPTS)
    if message_id == Ashita.Enum.Message.THIRD_EYE_ANTICIPATION then
        DB.Data.Update(DB.Update_Mode.INC, 1, audits, DB.Trackable.DEF_THIRD_EYE_ANTICIPATION, DB.Metric.HIT_COUNT)
        anticipation = true
    end
    return anticipation
end
...
H.Melee_Def.Guard = function(audits, damage, reaction_id)
    local guard = false
    DB.Data.Update(DB.Update_Mode.INC, 1, audits, DB.Trackable.DEF_GUARD, DB.Metric.ATTEMPTS)
    if reaction_id == Ashita.Enum.Reaction.GUARD then
        ...
        DB.Data.Update(DB.Update_Mode.INC, 1,      audits, DB.Trackable.DEF_GUARD, DB.Metric.HIT_COUNT)
        DB.Data.Update(DB.Update_Mode.INC, 1,      audits, DB.Trackable.DEF_MELEE, DB.Metric.HIT_COUNT)
        guard = true
    end
    return guard
...
H.Melee_Def.Block = function(audits, damage, reaction_id)
    local block = false
    DB.Data.Update(DB.Update_Mode.INC, 1, audits, DB.Trackable.DEF_SHIELD_BLOCK, DB.Metric.ATTEMPTS)
    if reaction_id == Ashita.Enum.Reaction.SHIELD_BLOCK then
        ...
        DB.Data.Update(DB.Update_Mode.INC, 1,      audits, DB.Trackable.DEF_SHIELD_BLOCK, DB.Metric.HIT_COUNT)
        DB.Data.Update(DB.Update_Mode.INC, 1,      audits, DB.Trackable.DEF_MELEE, DB.Metric.HIT_COUNT)
        block = true
    end
    return block
...
H.Melee_Def.Spikes = function(audits, result)
    ...
    if spike_effect then
        ...
        if spike_message == Ashita.Enum.Message.SPIKE_DMG then
            ...
            DB.Data.Update(DB.Update_Mode.INC, 1     , audits, DB.Trackable.SPELLS_SPIKE_DAMAGE, DB.Metric.HIT_COUNT)
            if spike_animation == Ashita.Enum.Effect_Animation.FIRE then
                ...
                DB.Catalog.Update_Metric(DB.Update_Mode.INC, 1, audits, DB.Trackable.SPELLS_SPIKE_DAMAGE, "Blaze Spikes", DB.Metric.HIT_COUNT)
            elseif spike_animation == Ashita.Enum.Effect_Animation.ICE then
                ...
                DB.Catalog.Update_Metric(DB.Update_Mode.INC, 1, audits, DB.Trackable.SPELLS_SPIKE_DAMAGE, "Ice Spikes", DB.Metric.HIT_COUNT)
            elseif spike_animation == Ashita.Enum.Effect_Animation.THUNDER then
                ...
                DB.Catalog.Update_Metric(DB.Update_Mode.INC, 1, audits, DB.Trackable.SPELLS_SPIKE_DAMAGE, "Shock Spikes", DB.Metric.HIT_COUNT)
            else
                DB.Data.Update(DB.Update_Mode.INC, damage, audits, DB.Trackable.TOTAL_DAMAGE, DB.Metric.TOTAL)
                DB.Data.Update(DB.Update_Mode.INC, damage, audits, DB.Trackable.TOTAL_DAMAGE_NO_SKILLCHAIN, DB.Metric.TOTAL)
...
H.Melee_Def.Additional_Effect = function(audits, value, animation_id, message_id, no_damage)
    ...
    if message_id == Ashita.Enum.Message.ENSPELL then
        ...
        if Res.Spells.Get_Enspell_Type(animation_id) then
            ...
            DB.Catalog.Update_Metric(DB.Update_Mode.INC, 1, audits, DB.Trackable.DEF_NUKING, enspell_name, DB.Metric.ATTEMPTS)
            DB.Catalog.Update_Metric(DB.Update_Mode.INC, 1, audits, DB.Trackable.DEF_NUKING, enspell_name, DB.Metric.HIT_COUNT)
        end
    end
end