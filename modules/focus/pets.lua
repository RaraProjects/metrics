...
Focus.Pets.Display = function(player_name)
    ...
    if UI.BeginTable("Pets Melee", 4, table_flags) then
        ...
        if ranged > 0 then
            ...
            UI.TableNextColumn() Column.Damage.By_Type(player_name, DB.Trackable.PET_RANGED_OVERALL, nil, nil, true)
            UI.TableNextColumn() UI.TextColored(Res.Colors.Basic.DIM, "---")
            Window_Manager.Table_Row_Color(row)
            row = row + 1
        end
        ...
        if nuke > 0 then
            ...
            UI.TableNextColumn() Column.Damage.By_Type(player_name, DB.Trackable.PET_NUKING, nil, nil, true)
            UI.TableNextColumn() UI.TextColored(Res.Colors.Basic.DIM, "---")
            Window_Manager.Table_Row_Color(row)
            row = row + 1
        end
