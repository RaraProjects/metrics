        ...
        for rank, data in ipairs(sorted_damage) do
            if rank <= Parse.Config.Rank_Cutoff() then
                ...
                if damage > 0 then
                    ...
                    UI.TableNextColumn() Column.Damage.By_Type_Metric(player_name, trackable, DB.Metric.ATTEMPTS_ON_USE)
        ...
        for rank, data in ipairs(sorted_damage) do
            if rank <= Parse.Config.Rank_Cutoff() then
                ...
                if damage > 0 then
                    ...
                    UI.TableNextColumn() Column.Damage.By_Type_Metric(player_name, trackable, DB.Metric.HITS_ON_USE)
                    UI.TableNextColumn() UI.TextColored(Res.Colors.Basic.DIM, "---")
                    UI.TableNextColumn() UI.TextColored(Res.Colors.Basic.DIM, "---")
                    Window_Manager.Table_Row_Color(1)
                    row = row + 1
